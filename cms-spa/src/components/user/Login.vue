    ...
    <div class="main">
        ...
        <form @submit.prevent="loginHandler">
            <div class="form-group">
                ...
                <input  type="email" class="form-control" id="eamil"  v-model="formData.email" required placeholder="Email" autofocus>
            </div>
            <div class="form-group">
                <label for="exampleInputPassword1">密码</label>
                <a class="pull-right" href="">忘记密码？</a>
                <input type="password" class="form-control" id="password"  v-model="formData.password" required placeholder="Password">
            </div>
            <div class="checkbox">
                <label>
            ...
            <p class="err-message" v-show="loginMessage">{{loginMessage}}</p>
            ...
            methods:{
               loginHandler:_.debounce(async function(){
                    try {
                        ...
                        this.loginMessage = ''
                        this.$router.push('/') //push('设置需要回到的页面') back()回到刚刚跳转过来的页面
                    }catch(err){
                        const{status} = err.response
                        switch (status) {
                            case 404:
                                this.loginMessage = '登录失败,请更换重试'
                                break
                        }
                    }
                },500)
                ...
                /*async loginHandler(){
                    try {
                        ...
                        const res = await axios.post('/api/session',formData)
                        this.loginMessage = ''
                        this.$router.back() //push('设置需要回到的页面') back()回到刚刚跳转过来的页面
                    }catch(err){
                        const{status} = err.response
                        switch (status) {
                            case 404:
